...
```{r}
# Plot 1: 
ggplot(data = combine, aes(y= Adj.Close, x = decade_decimalDate,group = decade)) + geom_line() + 
  facet_wrap(~decade) + 
...
```{r}
combine <- combine %>% 
  mutate(prediction_trend = predict(lm(logClose ~ poly(decimalDate,5)+ n)))
combine <- combine %>%
  mutate(prediction_trend2 = predict(loess(logClose ~ decimalDate, data = combine, span = 0.25)))
...
```{r}
combine <- combine %>%
  mutate(Detrend = logClose - prediction_trend2)
...
trend.mod <- lm(Adj.Close ~ poly(decimalDate,4,raw=TRUE), data= combine )
X = model.matrix(trend.mod)[,-1] #removes intercept column

combine  %>% pull(Adj.Close) %>%
ts(start = 1979, frequency = ) %>%
